...
namespace DotNet.Basics.Tests.Tasks
{
    ...
    public class RepeaterTaskTests
    {
        ...
        {
            Action action = () => Repeat.Task(async () =>
                  {
                      await Task.Delay(1.Milliseconds()).ConfigureAwait(false);//do nothing
                  })
                    .WithMaxTries(2)
        ...
        public void Task_IgnoreExceptionsEvenIfUntilIsNotReached_ActionIsInvokedFiveTimesAndNoExceptions()
        {
            var doCounter = 0;
            const int until = 5;
            var result = Repeat.Task(() => { doCounter++; throw new IOException("buuh"); })
                .WithNoRetryDelay()
                .WithMaxTries(until)
                .Until(() => false)
        ...
        public async Task TaskAsync_InvokeFiveTimes_ActionIsInvokedFiveTimes()
        {
            ...
            var result = await Repeat.TaskAsync(async () => invoked++)
                .WithNoRetryDelay()
                .WithPing(() => pinged++)
                 .Until(() => invoked == until)
                 .NowAsync();
